...
clients = com.get(B+'/api/clients').json(); check(len(clients)==2, 'com1 voit ses 2 clients')
c3 = adm.get(B+'/api/clients').json(); check(len(c3)==3, 'admin voit 3 clients')
# création demande
...
check(r.status_code==201, 'création demande'); d = r.json(); did = d['id']; check(d['statut']=='brouillon' and d['numero']==1, 'brouillon D-0001')
...
n = impl.get(B+'/api/notifications').json(); check(len(n)==1 and 'D-0001' in n[0]['sujet'], 'notification impl reçue')
mails = impl.get(B+'/api/mails').json(); check(len(mails)==1 and 'impl1@exemple.local' in mails[0]['contenu'], 'mail factice écrit')
# implantation
check(impl.get(B+'/api/demandes').json()[0]['id']==did, 'impl voit la demande')
...
# DT refuse
n = dt.get(B+'/api/notifications').json(); check(len(n)==1 and 'à valider' in n[0]['sujet'], 'notification DT')
...
stats = com.get(B+'/api/stats').json(); check(stats.get('chiffree')==1, 'stats')
r = com.post(B+f'/api/demandes/{did}/archiver'); check(r.ok and r.json()['archive']==1, 'archivage')
check(len(com.get(B+'/api/demandes').json())==0 and len(com.get(B+'/api/demandes?archive=1').json())==1, 'listes archive')
